...
The generator is based on WiiLink's MPL-2.0-licensed [ForecastChannel](https://github.com/WiiLink24/ForecastChannel) project. It uses Open-Meteo instead of an AccuWeather key. GitHub Actions requests a refresh at minute `:00` of every hour, and each generated file remains valid for one hour so the channel requests fresh weather on the hourly cycle. An earlier 4,038-location experiment was removed because the retail channel downloaded the data but rejected it at runtime.
